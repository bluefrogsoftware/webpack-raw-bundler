...
function MergeIntoFile(options) {
  ...
  if (includeFilePaths) CommentTags = options.commentTags || CommentTags;
...
function isExcluded(aString, currentBundleOptions) {
  var isIncluded =
    currentBundleOptions && currentBundleOptions.match
...
var walkSync = function(dir, filelist, ext, currentBundleOptions) {
  ...
  files.forEach(function(file) {
    if (fs.statSync(path.join(dir, file)).isDirectory()) {
      ...
      if (
        file.match(new RegExp('.' + ext + '$')) &&
        !isExcluded(file, currentBundleOptions)
      )
...
function mergeFiles(
  ...
  if (!ind) ind = 0;
  if (!AllowDuplicatesInBundle && list[ind] && BeenHere[list[ind]]) ind++;
  else BeenHere[list] = 1;
  ...
  if (
    ...
    (currentBundleOptions ||
      typeof list == 'string' ||
      typeof list[ind] == 'string')
  ) {
    var useThisString = currentBundleOptions
      ? currentBundleOptions.path
      : typeof list == 'string'
      ? list
      : list[ind];
    ...
    if (Matched) {
      ...
      list = walkSync(Matched[1], [], FileExt, currentBundleOptions).concat(
        list.slice(ind + 1),
      );
      ind = 0;
  ...
  var curfile = currentBundleOptions
    ? currentBundleOptions.path
    : typeof list == 'string'
    ? list
    : list[ind];
  ...
  if (printProgress)
    console.log(
      'Bundling: ' + curfile + '[' + (ind + 1) + '/' + list.length + ']',
    );

  if (typeof curfile == 'undefined') return callback(null, '');
  ...
  fs.readFile(curfile, Encoding, (err, body) => {
    ...
    var nextOptions =
      list[ind + 1] && list[ind + 1].path ? list[ind + 1] : null;
...
MergeIntoFile.prototype.apply = function(compiler) {
  ...
  compiler.hooks.emit.tapAsync('Visualizer', (compilation, callback) => {
    ...
    Bundles.forEach(function(filename) {
      if (printProgress) console.log('Starting to bundle: ' + filename);
